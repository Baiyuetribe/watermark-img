...
if ($_COOKIE["WmEnable"] == "on"){
    ...
    $url = $_COOKIE["WmUrl"] ?? "https://s2.ax1x.com/2019/02/14/kB6z9K.png";
    $opacity = $_COOKIE["WmOpacity"] ?? 100;
...
echo json_encode(dirname('http'.($_SERVER['HTTPS'] == 'on' ? 's' : '').'://'.$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']).'/'.$WmImg);
